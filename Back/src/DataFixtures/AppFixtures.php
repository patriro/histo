...
class AppFixtures extends Fixture
{
    ...
    {
        $this->loadLegalStatus($em);
        $this->loadCompanies($em);

        $em->flush();
    }

    private function loadLegalStatus(ObjectManager $em): void
    {
        $labelsLegalStatus = [
        ...
        foreach ($labelsLegalStatus as $labelLegalStatus) {
            $legalStatus = new LegalStatus();
            $legalStatus->setLabel($labelLegalStatus);
            $em->persist($legalStatus);
        }
    }

    private function loadCompanies(ObjectManager $em): void
    {
        ...
        for ($i=0; $i < 10; $i++) {
            ...
            $legalStatus->setLabel($legalStatuses[$i]);

            $address->setCity($faker->city());
            $address->setPostCode($faker->numberBetween(1, 99000));
            $address->setRoadType($faker->streetPrefix());
            $address->setNumber($faker->numberBetween(1, 200));
            $address->setRoadName($faker->name());

            $company->setName($faker->company());
            $company->setSiren($faker->siren());
            $company->setCapital($faker->numberBetween(10000, 10000000));
            $company->setRegistrationDate($faker->datetime());
